...
const Table = ({
    ...
    const { edit, editImage } = useType()
    const { edit: editBlog, editImage: editImageBlog } = useBlog()

    const handleEdit = async (e, slug, formData, setFormData) => {
        e.preventDefault()
    ...
    const handleEditImage = async (e, slug, formData, inputRef) => {
        e.preventDefault()
                ...
                <tbody>
                    {datas?.map((data, index) => (
                        <tr
                            ...
                            <td className="w-10">{data?.id}</td>
                            {name !== 'portfolios' && (
                                <>
                                    <td
                                        className={`align-middle ${data?.name || data?.title ? 'visible' : 'invisible'} w-32`}>
                                        <div className="w-32">
                                    ...
                                    <td
                                        className={`${data?.subname || data?.subtitle ? 'visible' : 'invisible'} align-middle w-40`}>
                                        <div className="w-40">
                                            {data.subname
                                                ? data.subname
                                                : data?.subtitle}
                                    ...
                                    <td
                                        className={`${data.description || data.content ? 'visible' : 'invisible'} w-72 `}>
                            ...
                            <td className="md:w-full w-32">
                                <div className="flex flex-col items-center justify-center w-32 md:w-full">
                                    <figure className="relative w-20 h-20 md:w-44 md:h-44">
                                        <Image
                                            fill
                                ...
                                <td className="w-32">
                                    <div className="flex flex-col items-center justify-center max-h-32 md:max-h-56">
                                        <div className="overflow-y-auto">
                                            {data?.setupImages?.length > 0 ? (
                                                data.setupImages.map(
                                                    (img, i) => (
                                                        <>
                                                            <figure className="relative w-20 h-20 md:w-44 md:h-44 mt-5">
                                                                <Image
